...
// Mirror container's transition-relevant class + inline transform/opacity onto
// the WebGL canvas so the 3D layer animates in lockstep with the DOM container.
// Call after any mutation to _container.classList or _container.style.transform
// in the transition flows.
function _syncGL3D() {
  const gl = _gl3d();
  if (!gl || !_container) return;
...
export function flyIntoPlanet(node, callback) {
  ...
  const targetScale =
    (Math.min(layoutW, layoutH) / (node.radius * zoom * 2)) * 0.6;

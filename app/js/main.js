...
async function loadProductData() {
  ...
  const [configModule, dataModule] = await Promise.all([
    import(`${productsBase}/config.js?v=18`),
    import(`${productsBase}/data.js?v=18`),
  ]);
  ...
  try {
    const iconsModule = await import(`${productsBase}/icons.js?v=18`);
  ...
  // Load tours (optional)
  try {
    const tourModule = await import(`${productsBase}/tour-data.js?v=18`);
  ...
  try {
    const feedbackModule = await import(`${productsBase}/feedback.js?v=18`);
  ...
  try {
    const aiContextMod = await import(`${productsBase}/ai-context.js?v=18`);
...
const loadEntities = async () => {
  try {
    const module = await import(`${productsBase}/entities.js?v=18`);

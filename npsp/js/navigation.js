...
function renderEntityGrid(component, entityType, pid) {
  ...
  const typeConfig = {classes:{icon:'{ }',color:'rgba(77,139,255,',badgeClass:'badge-class'},objects:{icon:'\u{1F5C3}',color:'rgba(34,197,94,',badgeClass:'badge-object'},triggers:{icon:'\u26A1',color:'rgba(239,68,68,',badgeClass:'badge-trigger'},lwcs:{icon:'\u{1F9E9}',color:'rgba(168,85,247,',badgeClass:'badge-lwc'},metadata:{icon:'\u2699',color:'rgba(245,158,11,',badgeClass:'badge-metadata'}};
...
function renderClassDetail(entity) {
  ...
  let h = `<div class="entity-detail-header"><div class="entity-detail-icon badge-class">{ }</div><div><h2 class="entity-detail-name">${entity.name}</h2><span class="entity-detail-type" style="background:${tc.bg};color:${tc.color}">${tc.label}</span>${entity.linesOfCode?`<span class="entity-detail-meta">${entity.linesOfCode} lines of code</span>`:''}</div></div>`;
...
function renderObjectDetail(entity) {
  let h = `<div class="entity-detail-header"><div class="entity-detail-icon badge-object">\u{1F5C3}</div><div><h2 class="entity-detail-name">${entity.label||entity.name}</h2><span class="entity-detail-meta" style="display:block;margin-top:2px">${entity.name}</span><span class="entity-detail-meta">${entity.fieldCount||0} fields</span></div></div>`;
...
function renderTriggerDetail(entity) {
  let h = `<div class="entity-detail-header"><div class="entity-detail-icon badge-trigger">\u26A1</div><div><h2 class="entity-detail-name">${entity.name}</h2><span class="entity-detail-meta">Trigger on ${entity.object}</span></div></div>`;
...
function renderLwcDetail(entity) {
  let h = `<div class="entity-detail-header"><div class="entity-detail-icon badge-lwc">\u{1F9E9}</div><div><h2 class="entity-detail-name">${entity.name}</h2><span class="entity-detail-type" style="background:rgba(168,85,247,0.1);color:#a855f7">Lightning Web Component</span></div></div>`;
...
function renderMetadataDetail(entity) {
  let h = `<div class="entity-detail-header"><div class="entity-detail-icon badge-metadata">\u2699</div><div><h2 class="entity-detail-name">${entity.name}</h2><span class="entity-detail-type" style="background:rgba(245,158,11,0.1);color:#f59e0b">Custom Metadata Type</span>${entity.recordCount?`<span class="entity-detail-meta">${entity.recordCount} records</span>`:''}</div></div>`;

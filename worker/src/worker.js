...
// products. Shared by every products/*/config.js via aiWorkerUrl.
//
// Contract:
//   POST /            { question, systemContext, faqMatches?, searchMatches } -> { answer }
//   POST /feedback     { question, rating: "up"|"down", reason?, comment? }   -> { ok: true }
//
// Transcribed verbatim (2026-08-11) from the live Cloudflare dashboard "Edit
// Code" source — this is the first time it has been under version control.
// No behavior changes. See ../README.md for required bindings.
  ...
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      const faqContext = (faqMatches || [])
        ...
        .map((f) => `Q: ${f.q}\nA: ${f.a}`)
        .join("\n\n");
      const searchContext = (searchMatches || [])
        .slice(0, 5)
        .map((s) => `- ${s.name} (${s.type}): ${s.desc}`)
        .join("\n");
      ...
      const messages = [
        {
          role: "system",
          content:
            (systemContext || "You are an expert on Salesforce NPSP.") +
  ...
  try {
    ...
    ctx.waitUntil(
      logToSheets(
        ...
        null,
        null,
        { type: "feedback", rating, reason: safeReason, comment: safeComment },
        queryHash,
      ),
    );
